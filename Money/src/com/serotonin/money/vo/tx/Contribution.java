...
public class Contribution extends Transaction {
    ...
    public Contribution(int id, int accountId, Date date, double amount) throws TransactionException {
        this(id, accountId, date, new BigDecimal(amount));
    }

    public Contribution(int id, int accountId, Date date, BigDecimal amount) throws TransactionException {
        if (!isGTZero(amount))
